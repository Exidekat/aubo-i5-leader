...
def test_process_demo(real_connection, com_port=None):
    ...
    try:
        if real_connection:
            if libpyauboi5 is None:
                logger.error("Cannot establish real connection: 'libpyauboi5' module not found.")
                return
...
def choose_com_port(dev_port):
    ...
    Args:
        dev_port (str): The device port specified via the '--dev' argument.

    Returns:
        str: The selected serial port device name.
    """
    if dev_port:
        ports = [port.device for port in list_ports.comports()]
        if dev_port in ports:
            return dev_port
        else:
            print(f"Error: Port '{dev_port}' not found.")
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dev', type=str, help='Specify the serial port for the Feetech motor board.')
